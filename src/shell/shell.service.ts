...
import { ApplicationType, GitRepoReq } from './git-repo.req.dto';
...
export class ShellService implements OnModuleInit {
    ...
    async onModuleInit() {
        Promise.all(
            this.configService.config.nodes.map((conn) =>
    ...
    async getShellByHostname(host: string) {
        const node = this.configService.config.nodes.find(
            (p) => p.host === host,
        );
    ...
    async dockerIsInstalled(
        client: SshExtended,
    ): Promise<{ isInstalled: boolean; version: string }> {
        const result = await this.runCommand('docker -v', client);

        return {
            isInstalled: result.outputs[0]?.startsWith('v') || false,
            version: result.outputs[0],
        };
    }

    async nodeIsInstalled(
        client: SshExtended,
    ): Promise<{ isInstalled: boolean; version: string }> {
    ...
    async npmIsInstalled(
        client: SshExtended,
    ): Promise<{ isInstalled: boolean; version: string }> {
        const result = await this.runCommand('npm -v', client);
        return {
            isInstalled: result.outputs[0]?.split('.')?.length === 3 || false,
            version: result.outputs[0],
        };
    }
    ...
    async updateApt(client: SshExtended): Promise<ShellCommandResultDto> {
        ...
        if (result.code) throw new Error(JSON.stringify(result, null, 2));
    ...
    async installFreshClientAPI(client: SshExtended): Promise<void> {
        ...
        await this.runCommand(
            'npm i --no-audit --no-progress -g pm2 && pm2 startup',
            client,
        );
        ...
        await this.runCommand(
            'git clone https://github.com/shellops/shellops-api.git ~/shellops-api',
            client,
        );

        await this.runCommand(
            'cd ~/shellops-api && npm i --no-audit --prefer-offline --no-progress',
            client,
        );
        ...
        await this.runCommand(
            'cd ~/shellops-api && pm2 start dist/main.js --name shellops-api',
            client,
        );
    ...
    async installDockerAptKey(client: SshExtended): Promise<void> {
        const aptKeyInstallResult = await this.runCommand(
    ...
    async uninstallDocker(client: SshExtended): Promise<void> {
        await this.runCommand(
            'sudo apt remove docker-ce containerd.io -y',
            client,
        );
    ...
    async installDocker(client: SshExtended): Promise<ShellCommandResultDto> {
        const depInstallResult = await this.runCommand(
        ...
        const installDockerResult = await this.runCommand(
    ...
    async installPoste(client: SshExtended): Promise<void> {
        ...
        await this.runCommand(
            `docker run -d --name poste --restart always \
 -p 110:110 \
 -p 143:143 \
 -p 2096:80 \
 -p 25:25 \
 ...
 analogic/poste.io:latest`,
    ...
    async latestNodeLtsAvailable(
        client: SshExtended,
    ): Promise<ShellCommandResultDto> {
    ...
    async installNodeLTS(client: SshExtended): Promise<void> {
        ...
        const {
            outputs: [latestNodeVersion],
        } = await this.latestNodeLtsAvailable(client);
        const { version: currentNodeVersion } = await this.nodeIsInstalled(
            client,
        );
    ...
    async runCommand(
        ...
        const result = await client.execCommand(command, {
            ...
            onStdout: (_chunk) => {
                ...
                Logger.verbose(
                    `STDOUT(${client.config.host}) -> ${JSON.stringify(chunk)}`,
                );
            },
            onStderr: (_chunk) => {
                const chunk = trim(_chunk.toString(), ' ');
                ...
                Logger.verbose(
                    `STDERR(${client.config.host}) -> ${JSON.stringify(chunk)}`,
                );
            },
        });
    ...
    async installFromGitRepo(client: SshExtended, gitRepoReg: GitRepoReq) {
        const dockerIsInstalled = await this.dockerIsInstalled(client);
        if (!dockerIsInstalled.isInstalled) {
            throw new Error('docker is not installed yet');
        }
        switch (gitRepoReg.application_type) {
            case ApplicationType.NODE_APPLICATION:
                await this.checkNpmAndNode(client);
                await this.cloneProject(client, gitRepoReg);
                const repoDir = this.getRepoDir(gitRepoReg.url);
                await this.runNodeApplication(
                    repoDir,
                    gitRepoReg.command,
                    client,
                );
    ...
    async runNodeApplication(
        repoDir: string,
        command: string,
        client: SshExtended,
    ) {
        await this.runCommand(
            `cd ~/${repoDir} && npm i --no-audit --prefer-offline --no-progress && npm ${command}`,
            client,
        );
    }

    getRepoDir(url: string) {
        return url.split('/').slice(-1)[0];
    }
    async cloneProject(client: SshExtended, gitRepoReg: GitRepoReq) {
        await this.runCommand(`git clone ${gitRepoReg.url} ~/`, client);
    }
    async checkNpmAndNode(client: SshExtended) {
        if (
            !(await this.nodeIsInstalled(client)) ||
            !(await this.npmIsInstalled(client))
        )
            throw new Error('npm or node not installed');
    }
}
